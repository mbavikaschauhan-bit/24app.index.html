...
(function() {
    ...
    const renderStatementContentWithExits = async (trades, ledger, statementBody, filters, partialExitsMap) => {
        try {
            const tradeRows = await Promise.all(trades.map(async t => {
                ...
                const net = closed ? window.calculateNetPnl(t) : 0;
            ...
            const rowsHtml = allRows.map(row => {
                ...
                return `
                    <tr class="border-b hover:bg-gray-50" style="border-color: var(--border-color);">
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.date}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.status}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.entryPrice}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.entryQty}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.entryDate}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.stopLoss}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.targetPrice}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.exitPrice}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.exitQty}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.exitDate}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.type}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.symbol}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.strategy}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.segment}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.direction}</td>
                        ...
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.winLoss}</td>
                        <td class="px-3 py-2 text-sm">${isDeposit || isWithdrawal ? formatAmount(row.amount) : '—'}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.outcomeSummary}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.emotionalState}</td>
                        <td class="px-3 py-2 text-sm" style="color: var(--text-primary);">${row.notes}</td>
                    </tr>
                `;
            }).join('');
    ...
    function renderDashboardUI(data) {
      try {
        ...
        // Account summary items (defensive)
        const accountSummaryContainer = document.getElementById('db-account-summary');
        if (accountSummaryContainer && summary.accountValue !== null) {
          // we keep the internal markup intact — this function updates known nodes only
          // For now, update the first item if present (NET ACCOUNT VALUE)
          const firstItem = accountSummaryContainer.querySelector('div p.text-2xl');
          if (firstItem) firstItem.textContent = formatCurrencySafe(summary.accountValue);
        }
